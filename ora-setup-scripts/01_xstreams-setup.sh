...
sqlplus sys/Admin123@//localhost:1521/ORCLCDB as sysdba <<- EOF
  ...
  GRANT SELECT ANY TRANSACTION TO c##xstrm CONTAINER=ALL;
  GRANT LOGMINING TO c##xstrm CONTAINER=ALL;

  GRANT CREATE TABLE TO c##xstrm CONTAINER=ALL;
  GRANT LOCK ANY TABLE TO c##xstrm CONTAINER=ALL;
  GRANT CREATE SEQUENCE TO c##xstrm CONTAINER=ALL;

  GRANT EXECUTE ON DBMS_LOGMNR TO c##xstrm CONTAINER=ALL;
  GRANT EXECUTE ON DBMS_LOGMNR_D TO c##xstrm CONTAINER=ALL;

  GRANT SELECT ON V_$LOG TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$LOG_HISTORY TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$LOGMNR_LOGS TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$LOGMNR_CONTENTS TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$LOGMNR_PARAMETERS TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$LOGFILE TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$ARCHIVED_LOG TO c##xstrm CONTAINER=ALL;
  GRANT SELECT ON V_$ARCHIVE_DEST_STATUS TO c##xstrm CONTAINER=ALL;
  exit;
EOF
